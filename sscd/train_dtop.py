...
class SSCD(pl.LightningModule):
    ...
    def on_train_end(self):
        ...
        # modi
        path = os.path.join(self.output_path, f"woanyreg_tr20k[model_{self.model_idx}]_{self.backbone}_{self.blk}_{self.orthogonality}_{self.lamb}_{self.token}_{self.desc_size}dims_model_torchscript.pt")      
...
def main(args):
	...
	# 		                project='exp_comp')
    wandb_logger = WandbLogger(name=f'woanyreg_tr20k[model_{args.model_idx}]_{args.backbone}_{args.blk}_{args.orthogonality}_{args.lamb}_{args.token}_{args.desc_size}_{c_date}_{c_time}', 
			                project='exp_ortho_3')

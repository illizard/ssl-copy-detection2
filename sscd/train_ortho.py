...
from utils.unif_w_check import compute_weight_orthogonality
...
class SSCD(pl.LightningModule):
    ...
    def loss(self, embeddings, instance_labels, w_dict): # modi 231003
        ...
        loss = infonce_loss + lamb*orthogonal_loss# modi 231003
    ...
    def on_train_end(self):
        ...
        # modi
        path = os.path.join(self.output_path, f"allortho_onlyorthoreg_sf1k[model_{self.model_idx}]_{self.backbone}_{self.blk}_{self.orthogonality}_{self.lamb}_{self.token}_{self.desc_size}dims_model_torchscript.pt")      
...
def main(args):
	...
	# 		                project='exp_comp')
    wandb_logger = WandbLogger(name=f'allortho_onlycont[model_{args.model_idx}]_{args.backbone}_{args.blk}_{args.orthogonality}_{args.lamb}_{args.token}_{args.desc_size}_{c_date}_{c_time}', 
			                project='exp_test')
